...
import { treeselect as menuTreeselect, roleMenuTreeselect } from '@/api/ruoyi/menu'
import { addRole, changeRoleStatus, dataScope, delRole, deptTreeSelect, getRole, listRole, updateRole } from '@/api/ruoyi/role'
...
function handleAuthUser(row) {
  router.push(`/role/roleAuthUser/${row.roleId}`)
}
/** 查询菜单树结构 */
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-tooltip v-if="scope.row.roleId !== 1" content="修改" placement="top">
            <el-button v-auth="['system:role:edit']" link type="primary" @click="handleUpdate(scope.row)">
              <template #icon>
                <FaIcon name="i-ep:edit" />
              </template>
            </el-button>
          ...
          <el-tooltip v-if="scope.row.roleId !== 1" content="删除" placement="top">
            <el-button v-auth="['system:role:remove']" link type="primary" @click="handleDelete(scope.row)">
              <template #icon>
                <FaIcon name="i-ep:delete" />
              </template>
            </el-button>
          ...
          <el-tooltip v-if="scope.row.roleId !== 1" content="数据权限" placement="top">
            <el-button v-auth="['system:role:edit']" link type="primary" @click="handleDataScope(scope.row)">
              <template #icon>
          ...
          <el-tooltip v-if="scope.row.roleId !== 1" content="分配用户" placement="top">
            <el-button v-auth="['system:role:edit']" link type="primary" @click="handleAuthUser(scope.row)">
              <template #icon>
          ...
          <template #label>
            <span>
              <el-tooltip content="控制器中定义的权限字符，如：@PreAuthorize(`@ss.hasRole('admin')`)" placement="top">
                <el-icon><FaIcon name="i-ep:question-filled" /></el-icon>
              </el-tooltip>
              权限字符
            </span>
          </template>
